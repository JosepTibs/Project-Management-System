...
export default function ProjectSetupSheet({
    ...
    const totalWorkItems = milestones.reduce(
        (sum, milestone) =>
            ...
            milestone.groups.reduce(
                (groupSum, group) =>
                    groupSum + group.work_items.length,
                0,
            ),
        0,
    );
    ...
    function addMilestone() {
        ...
        setExpandedMilestones((previous) =>
            new Set(previous).add(String(newId)),
        );
    }
    ...
    function addWorkItem(
        ...
        setExpandedWorkItems((previous) =>
            new Set(previous).add(
                `${milestoneIndex}-${groupIndex}-${newId}`,
            ),
        );
    }
    ...
    function jumpToIssue(issueKey: string) {
        const issue = collectIssues().find(
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {count > 0 &&
                                        item <=
                                            step && (
                                            <span className="absolute top-2 right-2 text-[11px] font-medium text-amber-600">
                                                {
                                                    count
                                                }{' '}
                                                issue
